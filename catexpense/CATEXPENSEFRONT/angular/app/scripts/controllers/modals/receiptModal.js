...
angular.module( 'expenseApp.Controllers' )
  ...
  [
      ...
      function (
          ...
          */
          if ( Cache.getOrigin() == "ManagerTable" || Cache.getOrigin() == "FinanceTable" ) {
          ...
          $scope.$on( "confirmDeleteReceipt", function () {
              ...
              ReceiptService.deleteReceipt( receiptId, $scope.userReceipts[receiptIndexId].LineItemId ).then(
                function ( success ) {
                    ...
                    LineItemService.getLineItemsBySubmissionId( Cache.getSubmission().SubmissionId ).then( function ( LineItems ) {
                        $scope.userReceipts.splice( receiptIndexId, 1 );
                        var lineItems = LineItems.data;
                        submissions[Cache.getSubmissionIndex()].LineItems = lineItems;
                        ...
                        submissions[Cache.getSubmissionIndex()].allSubmissionReceipts = receipts;
                        if ( receipts.length == 0 ) {
                            submissions[Cache.getSubmissionIndex()].ReceiptPresent = false;
                        }
